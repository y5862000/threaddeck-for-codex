...
- Add **Fixed task** to the Codex task action. Assign a persistent local or remote task by UUID and keep it on the same key through renames and reordering; missing tasks show **Unavailable** without opening a replacement. The task picker distinguishes duplicate titles, handles separate inspector/action contexts, and preserves saved assignments through failed reads and reconnects.
