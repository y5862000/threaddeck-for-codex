...
3. Effort/Fast 버튼에 **Codex 재실행**이 표시될 때만 Codex를 한 번 종료하고 다시 엽니다. ThreadDeck은 Codex를 자동으로 종료하거나 재실행하지 않고 이미 준비된 `127.0.0.1` 렌더러 연결만 재사용합니다.
4. 기존 방식 폴백을 위해 아래 Codex 단축키 4개를 확인하고 마이크 버튼을 시험합니다.
