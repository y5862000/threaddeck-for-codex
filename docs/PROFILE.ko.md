...
이 페이지의 모든 작업 키도 같은 **Codex 작업** 액션이며 속성 검사기에서 상위 1~7을 각각 선택했습니다. 사용자 지정 배치에서는 복사본 하나를 더 놓고 상위 8을 고르면 됩니다. 특정 작업을 한 버튼에 유지하려면 **고정 작업**을 고른 뒤 작업 목록에서 선택하세요. 이름이나 순서가 바뀌어도 UUID로 같은 작업을 유지하며, 사라지거나 보관된 작업은 사용 불가로 표시합니다. 기존 프로파일의 현재·상위 작업 설정은 그대로 유지됩니다.
