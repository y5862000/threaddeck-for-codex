...
Every release also includes `threaddeck-for-codex-neo.streamDeckProfile` as a separate asset. Use that file only when you want to restore the recommended layout, import a second editable copy, or inspect the profile without reinstalling the plugin. The standalone profile still requires the ThreadDeck plugin for its Codex actions.
