...
| `micro-cdp.js` | Persistent renderer discovery, privacy-bounded read-only snapshots, native Micro commands, PTT, exact power selection, and six-slot switching |
| `micro-main-inspector.js` | Exact-PID bootstrap evaluation through a short-lived loopback Node inspector |
| `micro-prepared-bridge.js` | Startup-prepared, authenticated Unix-domain command transport into the existing Codex renderer |
...
`src/micro-cdp.js` accepts only a debugger endpoint bound to loopback, validates the endpoint and main `app://` renderer target, then communicates over its CDP WebSocket. When that persistent route exists, its periodic snapshot is mutation-free and returns only the active task identity, next-run Effort, Fast state, theme, six privacy-bounded slot rows, and capability booleans. It does not return prompt, response, queued-message, or transcript text.

Mutating Micro activation is lazy: only the first physical control that needs HID/PTT installs an in-memory renderer feature-gate override and announces a connected Micro device. Official keycap commands drive Fast, Side Chat, Send, and New Task; host PTT events drive microphone input; Effort requests one exact mounted power-controller selection rather than replaying a compact physical encoder; and `AG00`–`AG05` drive exact native slots. The override exists only in that renderer process.
...
`src/micro-bootstrap.js` observes the running Codex generation and reuses a healthy persistent loopback bridge when one is already available. It validates the explicit launch port first, then rechecks only loopback listeners owned by that exact Codex PID so an endpoint can recover even when its port is absent from the command line. It never sends a termination signal, launches Codex, or scans arbitrary ports. Only process-generation, persistent port, health, and timestamp data are stored under `~/Library/Application Support/ThreadDeck`.

When an ordinary Codex launch has no persistent renderer endpoint, plugin startup prepares the command-only route before the first physical input. `src/micro-main-inspector.js` resolves the exact Codex main PID, refuses port 9229 if another PID owns it, sends `SIGUSR1` to that Codex process, and accepts only the resulting loopback Node target. It then installs the `src/micro-prepared-bridge.js` server in Codex main-process memory and closes an inspector opened by ThreadDeck after the short bootstrap window. The server exposes only newline-framed, size-bounded renderer evaluations over a randomized Unix-domain socket with mode `0600` and a 256-bit token retained only in the two process memories. Button commands reuse that already-open socket and are serialized inside Codex; they do not reopen the inspector. The socket is replaced after a Codex process-generation change and removed when either endpoint closes cleanly. This route never foregrounds or restarts Codex and is never used by the periodic read-only observer. If preparation fails before delivery, only a safe legacy operation may run; an ambiguous post-delivery result remains fail-closed.
...
- While the Dashboard is visible, a lightweight 750 ms observer combines a read-only Micro identity only when a persistent endpoint exists with the existing local and Accessibility window probes. The command-only inspector is never opened by this timer. A manual in-app switch still reaches Current Task and every dependent control in under a second. Send, microphone, Effort, Fast, and Side Chat re-confirm the same identity before acting. Before a new Side Chat UUID exists, the existing provisional lease may focus only a geometrically verified right-side composer; it is replaced by the real UUID on discovery. Queue controls remain associated with their enclosing task pane so main-task and Side Chat counts decrement independently.
- At plugin startup and after a Codex App Server or renderer generation change, ThreadDeck reads `config.toml` and `models_cache.json` to prime the visible Effort catalog, then reconciles it with a trustworthy composer value. Every tap starts a 320 ms eased track transition and rapid input keeps repainting without waiting for Codex. After the input settles, ThreadDeck sends one exact model/Effort selection through the mounted power controller when that target is exposed; otherwise it opens `Advanced`, rescans the live account/model options, and selects only the final requested level. The exact Ultra warning handler and composer-focus restoration remain scoped to that final selection. Direction reverses at either endpoint. A 600 ms hold fires Fast immediately through the native command and cannot dispatch again on release.
...
- Active task timers and animation frames render at device-appropriate intervals. `src/device-frame-policy.js` maps the official Stream Deck device type plus reported rows/columns to a per-key and aggregate budget; the measured Neo lane uses 30 fps per animated key with a 90 fps aggregate ceiling, while unmeasured hardware starts at the public conservative cadence. `src/image-delivery.js` keeps an independent aggregate clock per device, coalesces each context to its newest frame, backs a lane off when the shared plugin socket accumulates data, and returns toward its model ceiling only after a quiet recovery window.
...
The plugin caches the last image for each context and avoids sending unchanged frames. Device connect/change/disconnect events update or reset only the affected delivery lane, so two attached Stream Decks do not consume one shared frame budget.
