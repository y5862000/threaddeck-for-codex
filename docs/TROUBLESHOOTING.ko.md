...
작업 버튼 음성 입력은 말하는 동안 작업 버튼을 계속 누르고 있어야 합니다. 버튼을 놓으면 ThreadDeck이 Codex 화면의 음성 입력 중지 컨트롤을 활성화하고, 받아쓰기가 안정될 때까지 기다린 뒤 일반 후속 요청을 제출합니다. 오류가 표시되면 **받아쓰기 시작**이 `Control+Shift+D`인지, Codex가 앞에 열려 있는지, 메시지 입력 영역이 보이는지 확인합니다. 음성 입력이 실제로 시작되지 않으면 이제 잘못된 녹음 상태를 계속 표시하지 않고 오류로 전환합니다.
