...
ThreadDeck은 하나의 다국어 Stream Deck 플러그인으로 배포됩니다. 액션 이름, 버튼 설정, 버튼 UI는 Stream Deck 앱 언어에 따라 영어·한국어·러시아어로 자동 표시됩니다. 지원하지 않는 언어는 영어로 표시되며 언어별 빌드를 따로 받을 필요가 없습니다.
...
5. **Codex → 설정 → 키보드 단축키**에서 다음 폴백 값을 지정하거나 확인합니다. 상시 Micro 렌더러 endpoint가 없어도 Codex를 별도 방식으로 실행하거나 재시작할 필요가 없습니다. 플러그인 시작 시 정확한 실행 중 Codex 프로세스에 전용 명령 소켓을 미리 준비하고, 평소 모니터링과 폴백은 로컬·손쉬운 사용 어댑터를 유지합니다.
...
이미 건강한 프로세스 소유 렌더러 endpoint가 있으면 재사용합니다. 없으면 플러그인 시작 시 정확한 Codex PID의 명령 전용 루프백 inspector를 잠시 열어 프로세스 메모리에 인증된 권한 `0600` Unix 도메인 소켓을 설치한 뒤 첫 물리 입력 전에 inspector를 닫습니다. 버튼은 이 준비된 소켓을 재사용하므로 입력 순간에 다시 연결하지 않습니다. 다른 프로세스가 초기 준비 포트를 사용 중이거나 실행 중인 Codex가 예상 렌더러를 노출하지 않으면 실패 폐쇄하고, 중복 실행이 안전한 동작만 검증된 폴백으로 넘깁니다. 이 연결 때문에 Codex를 재실행하거나 앞으로 가져오지 않습니다.

소스 체크아웃에서는 `pnpm run doctor`로 상시 Codex 렌더러 브리지가 연결됐는지, 아니면 명령이 짧은 정확한 프로세스 경로를 사용할지 확인할 수 있습니다. doctor 자체는 이 경로를 열지 않고 두 앱을 시작·종료·수정하지 않습니다. 자세한 해결법은 [문제 해결](TROUBLESHOOTING.ko.md)을 확인하세요.
