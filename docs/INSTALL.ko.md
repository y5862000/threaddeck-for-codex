...
5. Effort/Fast 버튼에 **Codex 재실행**이 표시될 때만 Codex를 종료한 뒤 평소처럼 한 번 실행합니다. ThreadDeck은 Codex를 자동으로 종료하거나 재실행하지 않고, 이미 준비된 건강한 루프백 Micro 연결만 재사용합니다.
...
Effort/Fast 버튼에 **Codex 재실행**이 표시될 수도 있습니다. 손쉬운 사용 문제가 아니라 현재 Codex 프로세스에 건강한 로컬 렌더러 연결이 없다는 뜻입니다. ThreadDeck은 현재 프로세스를 그대로 두므로 편한 때에 사용자가 직접 Codex를 종료하고 다시 여세요. 그동안 기존 방식 제어는 계속 사용할 수 있습니다.
