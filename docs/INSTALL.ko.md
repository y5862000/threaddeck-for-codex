...
5. **Codex → 설정 → 키보드 단축키**에서 다음 폴백 값을 지정하거나 확인합니다. 선택 기능인 Micro 렌더러 연결이 없어도 Codex를 재실행할 필요가 없습니다. ThreadDeck이 즉시 검증된 손쉬운 사용 어댑터로 제어하고, 현재 프로세스 소유의 건강한 루프백 endpoint를 계속 확인합니다.
...
Effort/Fast 버튼에는 기존 루프백 endpoint를 다시 확인하는 동안 잠시 **연결 중**이 표시될 수 있습니다. 계속 사용할 수 없으면 브리지 안내를 지우고 Codex를 재실행하지 않은 채 검증된 손쉬운 사용 어댑터로 전환합니다. 이후 건강한 endpoint가 나타나면 자동으로 다시 승격합니다.

소스 체크아웃에서는 `pnpm run doctor`로 Micro 연결됨·재연결 중·폴백 사용·중지 상태를 확인하는 읽기 전용 설치 진단을 실행할 수 있습니다. doctor는 두 앱을 시작·종료·수정하지 않습니다. 자세한 해결법은 [문제 해결](TROUBLESHOOTING.ko.md)을 확인하세요.
