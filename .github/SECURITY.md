...
- `~/Library/Application Support/ThreadDeck/codex-micro-bootstrap-v1.json` and `codex-micro-bridge.json` store only process generation, loopback port, health state, and numeric timestamps. ThreadDeck never terminates or relaunches Codex automatically. If the loopback bridge is unavailable, it leaves the running process untouched and asks the user to restart Codex.
