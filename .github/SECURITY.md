...
- `~/Library/Application Support/ThreadDeck/codex-micro-bootstrap-v1.json` and `codex-micro-bridge.json` store only process generation, loopback port, health state, and numeric timestamps. ThreadDeck never terminates or relaunches Codex automatically. It checks only loopback listeners owned by the exact running Codex process, reconnects a verified bridge when one exists, and otherwise uses the verified Accessibility adapter without asking for a Codex restart. A later healthy bridge is promoted automatically.
