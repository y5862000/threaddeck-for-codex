...
| Reasoning + Fast | Release before 0.6 s to move the next-run level (`LIGHT`–`ULTRA`) immediately | Rapid taps keep moving the hardware track, then a 1.1-second settle window applies only the final stopped level to Codex in one verified transaction. Direct changes in Codex are mirrored on this key. Missing options fail closed and endpoints reverse direction; both Codex's compact slider and Effort menu are supported. When Full access makes Codex show its exact Ultra warning, ThreadDeck verifies that dialog and chooses `Use Full access`—never the generic `Continue` path that changes permissions. At **0.6 s**, the key starts toggling next-run Fast mode immediately without waiting for release. The small bolt mirrors verified speed |
