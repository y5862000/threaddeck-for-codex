...
| 추론 강도 + Fast | 0.6초 전에 놓으면 다음 응답 단계(`LIGHT`–`ULTRA`)를 이동하며 첫 클릭부터 트랙이 즉시 반응 | 시작할 때 Codex의 읽기 전용 데스크톱 설정과 선택 모델 캐시에서 실제 노출 단계를 미리 읽되 선택적인 `Max`·`Ultra`가 있다고 추정하지 않음. 빠르게 연타하면 트랙은 매번 즉시 움직이고, 입력이 1.1초 멈춘 뒤 최종 왕복 위치만 한 번 적용·검증. 적용 시에는 현재 UI 목록을 다시 스캔해 최종 기준으로 삼으므로 모델·계정 메뉴가 달라져도 안전하게 보정. Codex에서 직접 바꾼 값도 이 버튼에 반영. 작은 슬라이더가 먼저 보이면 정확한 `Advanced`를 열어 나온 Effort 목록에서 선택하며 눈금을 추정하지 않음. Full access에서 Codex의 정확한 Ultra 경고가 뜨면 대화상자를 검증하고 `Use Full access`를 선택하며, 권한을 바꾸는 일반 `Continue` 경로는 누르지 않음. **0.6초** 이상 누르는 순간 버튼을 놓기 전에 다음 응답의 Fast mode 전환을 시작. 작은 번개는 확인된 속도 표시 |
