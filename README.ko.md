...
직접 고정한 원격 작업은 Codex의 UUIDv7 턴 ID에서 최근 턴 시작 시각을 복원하고, 명시적인 종료 수명 주기 이벤트 또는 실시간으로 확인한 `active` → `notLoaded` 전환에서만 타이머를 멈춥니다. 원격 요약의 업데이트 시각은 정렬에만 사용하며 완료 시각으로 쓰지 않습니다. 플러그인을 새로 시작해 신뢰할 종료 경계를 얻을 수 없으면 짧은 시간을 만들어 내지 않고 소요 시간을 알 수 없음으로 표시합니다. 추론 강도는 포커스된 창이 정확한 원격 작업과 일치할 때 보이는 작성기의 실제 설정값(또는 명시적인 원격 메타데이터)만 사용합니다. Codex가 값을 노출하지 않으면 중간으로 추정하지 않고 트랙을 비워 둡니다.
