...
- **실시간 작업 카드** — 현재 단계, 제목, 고정 여부, 대기 개수, 진행·완료 시간, 추론 강도, 고속·일반 서비스 단서.
...
- **완료 피드백** — 화면의 모든 ThreadDeck 소유 버튼이 첫 완료 프레임을 함께 표시하고, 해당 작업 버튼은 더 강한 초록 펄스를 이어갑니다.
- **작업 흐름 제어** — Codex Fast mode 전환, 새 작업, 사이드챗, 보내기, 앱 전환, 페이지 이동, 미디어 키, 선택 기능인 주간 한도 링.
...
4. **시스템 설정 → 개인정보 보호 및 보안 → 손쉬운 사용**에서 **Stream Deck**을 허용한 뒤 Stream Deck을 완전히 종료하고 다시 실행합니다.
...
|---|---|---|
| 현재/마지막 전환 작업(1번 슬롯) | 현재 작업 또는 ThreadDeck이 전환 성공을 확인한 마지막 작업 열기 | **0.55초** 이상 누르면 해당 작업에서 음성 입력 시작; 놓으면 받아쓰기, 자동 제출, 작성창 초기화 확인 |
| 작업 2~8 | 로컬·원격·사이드챗 작업 열기를 시작 | **0.55초** 이상 누르면 해당 작업에서 음성 입력 시작; 놓으면 받아쓰기, 자동 제출, 작성창 초기화 확인 |
| 전용 마이크 | 즉시 음성 입력을 시작하고 지원되는 오디오 출력 미디어 앱을 잠시 멈춤 | 말하는 동안 계속 누름; 놓으면 미디어를 재개하고 받아쓴 내용을 작성창에 남기되 **자동 제출하지 않음** |
| 보내기 | 0.6초 전에 놓으면 Return | **0.6초**가 되면 파란색으로 전환; 놓으면 Command+Return |
| 앱 실행 | 짧게 누르면 지정 앱을 열거나 앞으로 가져옴 | 길게 누르면 해당 앱 종료; 임계 시간과 실제 그림은 Stream Deck이 관리 |
| 주간 한도 | — | 놓으면 CodexBar 값을 즉시 새로 고침 |
| 새 작업 / 사이드챗 | — | 놓으면 `⌥⌘O` / `⌥⌘S` 실행 |
| Fast mode | — | 놓으면 현재 Codex 작업의 작성창에서 Fast mode 전환; 확인된 상태만 버튼에 표시 |
| 앱 전환 / 미디어 | 누르는 순간 실행 | 별도 길게 누르기 동작 없음 |
| 이전 / 다음 페이지 | — | 놓으면 ThreadDeck의 3개 페이지를 순환 |

대시보드의 1번 작업 버튼은 의도적으로 안정적으로 유지됩니다. 현재 식별된 Codex 작업을 나타내고, ThreadDeck이 작업 전환 성공을 확인한 뒤에는 마지막으로 성공한 정확한 대상을 유지합니다. 실패하거나 모호한 전환은 이 버튼을 바꾸지 않습니다.
...
짧게 누르면 해당 작업을 엽니다. **0.55초 이상** 길게 누르고 `말하는 중` 표시가 나타난 뒤 말하세요. 느린 원격 전환에서는 정확한 작업과 작성창을 확인하는 동안 먼저 `전환 준비`가 표시됩니다. 버튼을 놓으면 녹음을 끝내고, 받아쓴 내용이 안정될 때까지 기다린 다음 Codex의 화면 보내기 컨트롤을 활성화합니다. 작성창이 실제로 비워졌음을 확인한 뒤에만 `전송 완료`를 표시합니다. 지원되는 활성 미디어는 macOS의 정상 미디어 명령으로 잠시 멈췄다가 재개됩니다. 선택한 작업 카드에 모든 단계가 나타납니다. 작업 전환과 작성창 활성화는 손쉬운 사용 포커스와 키보드로 처리하므로 마우스 포인터가 움직이지 않습니다.
...
| 예시 | 의미 |
|---|---|
| <img src="docs/media/working-task-key.png" width="104" alt="진행 중인 작업 버튼"> | 헤더는 현재 단계를 표시합니다. 핀은 제목 앞에, 시간은 1초마다 갱신되고, 트랙은 신뢰할 수 있는 추론 강도를 나타냅니다. 번개는 고속 서비스를, 주황 `+N`은 관찰된 대기 후속 요청 개수를 뜻합니다. |
...
작업 상태는 진행 중인 파랑·보라, 현재 단계, 고정 핀, 움직이거나 멈춘 시간, 주황 대기 개수, 완료 체크, 오류·중지, 초록 완료 펄스로 일관되게 표시됩니다. 라이트 모드에서도 정보 순서는 같습니다.
...
고정하지 않은 원격 기록은 일반 버튼 슬롯을 차지하지 않습니다. 단, 1번 슬롯에는 현재 작업으로 정확히 확인됐거나 ThreadDeck이 마지막 전환 성공을 확인한 원격 작업 하나가 고정 여부와 관계없이 유지될 수 있습니다. 내부 보조·검토 작업은 제목을 렌더러에 넘기기 전에 구조 메타데이터로 제외합니다. 보관된 영구 작업 ID가 프롬프트 기록을 통해 가짜 사이드챗으로 다시 들어오는 경로도 차단합니다.
...
원격 요약의 갱신 시각은 정렬에만 사용하며 완료 시각으로 만들지 않습니다. 턴의 UUIDv7 ID에서 시작 시각을 복원하고 명시적인 종료 이벤트 또는 실시간으로 확인한 active-to-not-loaded 전환에서만 소요 시간을 고정합니다. 새로 시작한 뒤 신뢰할 종료값이 없으면 임의의 짧은 시간 대신 알 수 없음으로 표시합니다. 중간 reasoning summary는 원문을 표시하거나 저장하지 않고 `계획 중`, `분석 중`, `구현 중`, `검증 중`, `실행 중`, `정리 중` 같은 개인정보 안전 단계로만 줄입니다. 추론 강도도 정확한 작업과 연결된 값을 Codex가 제공할 때만 표시하며, 없으면 트랙을 비워 둡니다.
...
1. **대시보드** — 한도, 현재/마지막 전환 성공 작업, Fast mode, 새 작업, 사이드챗, 마이크, 보내기, 뒤로가기. Fast mode는 `1,1`, 작업 버튼은 `0,1`에 있습니다.
2. **작업** — 현재/마지막 전환 작업, 작업 2~7번, 뒤로가기. 사용자 지정 레이아웃에는 액션 목록의 작업 8번도 추가할 수 있습니다.
...
| 데이터 소스 | 접근 | 목적 |
|---|---|---|
| `~/.codex`와 Codex Desktop 로그 | 읽기 전용 | 사용자 작업 제목, 고정, 캐시된 원격 요약, 수명 주기, 활동, 시간, 서비스 메타데이터, 임시 사이드챗 수명 주기 |
| CodexBar CLI | 선택 하위 프로세스 | 남은 주간 한도만 확인; CodexBar의 제공자 동작은 별개 |
| Stream Deck 플러그인 소켓 | 로컬호스트 | 버튼 이벤트 수신과 렌더링 이미지 전송 |
| macOS 손쉬운 사용과 Core Audio | 로컬 시스템 API | 키보드·미디어 동작, 작성창 확인, 대기 개수, 원격 선택, 음성 입력 중 오디오 처리 |
...
| 증상 | 먼저 확인할 것 |
|---|---|
| 어떤 버튼도 동작하지 않음 | 손쉬운 사용에서 Stream Deck을 다시 허용한 뒤 Stream Deck을 완전히 종료하고 재실행 |
...
- 지원되는 활성 미디어는 음성 입력 중 macOS의 정상 미디어 명령으로 잠시 멈추고, 마지막 음성 버튼을 놓은 뒤에만 재개됩니다. 브라우저의 미디어 세션은 여러 탭에 영향을 줄 수 있습니다.
