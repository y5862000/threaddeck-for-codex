...
| Fast mode | — | 놓으면 현재 Codex 작업의 작성창에서 Fast mode 전환; 아이콘은 확인된 고속·일반 상태를 표시하고 Codex가 쓸 수 있는 컨트롤을 노출하지 않을 때만 사용 불가로 표시 |
