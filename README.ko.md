...
1. **대시보드** — 주간 한도, 작업 1개, 새 작업, 사이드챗, 누르는 동안 음성 입력, 보내기, 앱 전환, 기본 뒤로가기.
2. **작업** — 작업 1~7번과 기본 뒤로가기. 플러그인에는 8번째 작업 액션도 포함되어 있어 원하는 레이아웃에 추가할 수 있습니다.
3. **미디어** — 이전 트랙, 되감기, 재생/일시정지, 앱 실행 4개, 기본 뒤로가기. 액션 목록에는 다음, 탐색, 음소거, 음량 조절도 포함되어 있습니다.
...
- Elgato 기본 탐색·앱 실행 버튼은 원래 렌더링과 동작을 유지하며 ThreadDeck 완료 효과는 적용되지 않습니다.
