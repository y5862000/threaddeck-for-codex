...
ThreadDeck은 Stream Deck Neo를 Codex의 물리 작업 모니터이자 컨트롤러로 바꿉니다. [Codex Micro](https://github.com/mpociot/codex-micro-stream-deck-emulator)가 보여준 소형 하드웨어 에이전트 흐름에서 영감을 받았지만, 자체 렌더러와 검증형 macOS 자동화를 쓰는 독립 구현이며 Codex Micro를 런타임 의존성으로 사용하지 않습니다.
...
3. 아래 Codex 단축키 3개를 확인하고 마이크 버튼을 시험합니다.
...
| 새 작업 / 사이드챗 | — | 놓으면 `⌥⌘O` / `⌥⌘S` 실행; 사이드챗은 보호된 식별 임대를 유지하며 전용 마이크만 임시 작성창 연결을 사용 |
| 추론 강도 + Fast | 0.6초 전에 놓으면 다음 응답 단계(`LIGHT`–`ULTRA`)를 이동하며, 최초의 정확한 목록 스캔 뒤부터 트랙이 즉시 반응 | 바꿀 때마다 현재 Codex 모델·계정에 실제로 보이는 단계를 다시 스캔하고 그 안에서만 이동하므로 선택적인 `Max`가 없으면 건너뛰고, 있을 때만 사용. 빠르게 연타하면 캐시한 정확한 트랙은 매번 움직이고, 입력이 1.1초 멈춘 뒤 최종 왕복 위치만 한 번 적용·검증하며 그사이 메뉴가 바뀌었으면 새 스캔 결과로 보정. Codex에서 직접 바꾼 값도 이 버튼에 반영. 작은 슬라이더가 먼저 보이면 정확한 `Advanced`를 열어 나온 Effort 목록에서 선택하며 눈금을 추정하지 않음. Full access에서 Codex의 정확한 Ultra 경고가 뜨면 대화상자를 검증하고 `Use Full access`를 선택하며, 권한을 바꾸는 일반 `Continue` 경로는 누르지 않음. **0.6초** 이상 누르는 순간 버튼을 놓기 전에 다음 응답의 Fast mode 전환을 시작. 작은 번개는 확인된 속도 표시 |
...
대시보드의 현재 작업 버튼은 Codex 앱의 활성 창에서 선택된 작업을 따라갑니다. Codex에서 직접 작업을 바꾸면 가벼운 활성 창 감시가 1초 안에 버튼을 갱신하며, 보내기·전용 마이크·추론 강도·Fast mode·사이드챗도 실행 직전에 같은 현재 작업을 다시 확인합니다. 보내기와 마이크는 확인된 작업의 작성창에 초점을 맞춘 뒤 동작합니다. 새 사이드챗은 그 작업을 부모로 기억하고, Codex가 새 사이드챗의 작업 식별자와 포커스를 실제로 노출할 때까지 전환 권한을 유지합니다. UUID가 아직 없는 짧은 구간에는 전용 마이크만 임시 대상을 사용할 수 있습니다. ThreadDeck이 손쉬운 사용 컨트롤과 창 내부 위치로 오른쪽 사이드챗 작성창을 확인한 뒤 녹음을 시작하고, 실제 UUID가 나타나면 같은 세션을 넘깁니다. 나머지 제어 버튼은 부모 작업으로 새지 않고 새 사이드챗 식별을 기다립니다. ThreadDeck 작업 버튼을 누른 경우에도 Codex가 대상 전환을 확인한 뒤 자연스럽게 갱신됩니다. 손쉬운 사용 값을 잠시 읽지 못하거나 제목이 모호하면 임의로 고르지 않고 마지막으로 검증한 현재 작업을 유지합니다. **상위 작업 1**은 별도의 선택 가능한 액션이므로 사용자 프로필에 두 버튼을 나란히 둘 수 있습니다.

추론 강도와 속도는 두 시점을 구분합니다. 진행 중인 작업 카드는 해당 응답이 시작될 때 확정된 Effort와 Fast/일반 설정을 끝까지 유지합니다. Codex 작성창이나 ThreadDeck 결합 버튼에서 값을 바꿔도 이미 진행 중인 응답 표시를 덮어쓰지 않습니다. 결합 버튼은 Codex에서 직접 바꾼 값까지 포함해 **다음 응답**에 적용될 현재 작성창 설정을 보여줍니다. 현재 Codex 대기열은 후속 요청 내용만 보관한 뒤 실제 실행 시점의 작성창 설정으로 새 턴을 시작하므로, 대기 중에 값을 바꾸면 그 요청이 시작될 때의 값이 적용됩니다. 실행이 시작되면 새 턴 메타데이터가 작업 카드 상단으로 넘어옵니다. 주황 `+N`은 각 대기 요청마다 고정된 설정이 있는 것처럼 추정하지 않고 개수만 표시합니다.
