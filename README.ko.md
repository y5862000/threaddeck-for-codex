...
- **작업 흐름 제어** — 새 작업, 사이드챗, 보내기, 앱 전환, 페이지 이동, 미디어 키, 선택 기능인 주간 한도 링.
...
|---|---|---|
| 작업 1~8 | 로컬·원격·사이드챗 작업 열기를 시작 | **0.55초** 이상 누르면 해당 작업에서 음성 입력 시작; 놓으면 받아쓰기, 자동 제출, 작성창 초기화 확인 |
...
짧게 누르면 해당 작업을 엽니다. **0.55초 이상** 길게 누르고 `말하는 중` 표시가 나타난 뒤 말하세요. 버튼을 놓으면 녹음을 끝내고, 받아쓴 내용이 안정될 때까지 기다린 다음 Codex의 화면 보내기 컨트롤을 활성화합니다. 작성창이 실제로 비워졌음을 확인한 뒤에만 `전송 완료`를 표시합니다. 지원되는 오디오 출력 미디어 프로세스는 누르는 동안 잠시 멈추고 놓을 때 재개됩니다. 선택한 작업 카드에 모든 단계가 나타납니다. 작업 전환과 작성창 활성화는 손쉬운 사용 포커스와 키보드로 처리하므로 마우스 포인터가 움직이지 않습니다.

### 전용 마이크: 초안만 받아쓰기

![전용 마이크를 누르는 동안 녹음하고 작성창에 초안을 남기는 흐름](docs/media/voice-hold-to-dictate.gif)

마이크 버튼은 누르는 즉시 녹음을 시작합니다. 말하는 동안 계속 누른 뒤 놓으세요. ThreadDeck은 받아쓰기를 끝내고, 잠시 멈춘 지원 미디어 프로세스를 재개한 뒤, 받아쓴 내용을 Codex 작성창에 남깁니다. 메시지를 **자동으로 전송하지 않습니다**. 내용을 확인한 뒤 보내기 버튼을 사용하세요. 화면에 추적 가능한 대상 작업이 있으면 해당 작업 카드에도 녹음 상태가 함께 나타날 수 있습니다.
...
| <img src="docs/media/completed-task-key.png" width="104" alt="완료된 작업 버튼"> | 체크와 멈춘 시간은 최근에 관찰한 턴이 완료됐다는 뜻입니다. 나중에 같은 작업을 다시 열어도 확정된 완료 시간은 덮어쓰지 않습니다. |
...
1. 고정·최근 **로컬 작업**;
2. Codex에서 사용자가 직접 고정한 **원격 작업**;
3. Codex 세션이 열려 있는 동안만 유지되는 임시 **사이드챗**.

고정하지 않은 원격 기록은 버튼 슬롯을 차지하지 않습니다. 내부 보조·검토 작업은 제목을 렌더러에 넘기기 전에 구조 메타데이터로 제외합니다. 보관된 영구 작업 ID가 프롬프트 기록을 통해 가짜 사이드챗으로 다시 들어오는 경로도 차단합니다.

원격 작업을 표시하려면 Codex에서 해당 컴퓨터를 한 번 열어 요약을 로컬에 동기화한 뒤, Stream Deck에 둘 작업만 직접 고정하세요. 버튼을 누르면 제목이 정확히 일치하는 Codex 사이드바 또는 통합 검색 결과를 키보드로 활성화해 컴퓨터와 작업을 함께 전환합니다. 제목이 같은 원격 결과가 여러 개면 임의로 선택하지 않고 `제목 중복`을 표시하므로 이름을 다르게 하거나 하나만 고정하세요.
...
1. **대시보드** — 한도, 작업 1개, 새 작업, 사이드챗, 마이크, 보내기, 앱 전환, 뒤로가기.
2. **작업** — 작업 1~7번과 뒤로가기. 사용자 지정 레이아웃에는 액션 목록의 작업 8번도 추가할 수 있습니다.
...
GIF 파이프라인은 Node.js와 macOS 기본 `sips`, Swift, ImageIO만 사용합니다. 외부 GIF 인코더나 복제한 제품 UI를 포함하지 않습니다.
...
- 지원 미디어 앱은 음성 입력 중 프로세스 단위로 잠시 멈춥니다. 오디오를 내는 브라우저 프로세스는 여러 탭에 영향을 줄 수 있습니다.
