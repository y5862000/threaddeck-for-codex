...
- **완료 피드백** — 대기 후속 요청이 없는 마지막 턴의 새 종료값이 확인된 뒤에만 화면의 모든 ThreadDeck 소유 버튼이 첫 완료 프레임을 함께 표시하고, 해당 작업 버튼은 더 강한 초록 펄스를 이어갑니다. 대기 명령 수정이나 실행 순서로의 전환은 완료로 세지 않습니다.
...
|---|---|---|
| 현재/마지막 전환 작업 | 현재 작업 또는 ThreadDeck이 전환 성공을 확인한 마지막 작업 열기 | **0.55초** 이상 누르면 해당 작업에서 음성 입력 시작; 놓으면 받아쓰기, 자동 제출, 작성창 초기화 확인 |
...
| Fast mode | — | 놓으면 현재 Codex 작업의 작성창에서 Fast mode 전환; 채운 초록 번개는 고속, 윤곽선 번개는 일반이며 상태에 문제가 있을 때만 경고 문구를 표시 |
| 앱 전환 / 미디어 | 누르는 순간 실행 | 별도 길게 누르기 동작 없음 |
| 이전 / 다음 페이지 | — | 놓으면 ThreadDeck의 3개 페이지를 순환 |

대시보드의 현재/마지막 전환 작업 버튼은 의도적으로 안정적으로 유지됩니다. 현재 식별된 Codex 작업을 나타내고, ThreadDeck이 작업 전환 성공을 확인한 뒤에는 마지막으로 성공한 정확한 대상을 유지합니다. 실패하거나 모호한 전환은 이 버튼을 바꾸지 않습니다. **상위 작업 1**은 별도의 선택 가능한 액션이므로 사용자 프로필에 두 버튼을 나란히 둘 수 있습니다. 기본 작업 페이지는 상위 작업 1로 시작하고 대시보드는 현재/마지막 전환 작업을 유지합니다.
...
고정하지 않은 원격 기록은 일반 상위 작업 슬롯을 차지하지 않습니다. 별도의 현재/마지막 전환 작업 액션만 예외입니다. 현재 작업으로 정확히 확인됐거나 ThreadDeck이 마지막 전환 성공을 확인한 원격 작업 하나는 이 버튼에 고정 여부와 관계없이 유지될 수 있습니다. 내부 보조·검토 작업은 제목을 렌더러에 넘기기 전에 구조 메타데이터로 제외합니다. 보관된 영구 작업 ID가 프롬프트 기록을 통해 가짜 사이드챗으로 다시 들어오는 경로도 차단합니다.
...
1. **대시보드** — 한도, 현재/마지막 전환 성공 작업, Fast mode, 새 작업, 사이드챗, 마이크, 보내기, 뒤로가기. Fast mode는 `1,1`, 현재/마지막 전환 작업은 `0,1`에 있습니다.
2. **작업** — 상위 작업 1~7과 뒤로가기. 상위 작업 8과 별도의 현재/마지막 전환 작업 액션은 사용자 지정 레이아웃의 액션 목록에서 추가할 수 있습니다.
