...
대시보드의 현재 작업 버튼은 Codex 앱의 활성 창에서 선택된 작업을 따라갑니다. Codex에서 직접 작업을 바꿔도 동기화되고, ThreadDeck 작업 버튼을 누른 경우에도 Codex가 대상 전환을 확인한 뒤 자연스럽게 갱신됩니다. 손쉬운 사용 값을 잠시 읽지 못하거나 제목이 모호하면 임의로 고르지 않고 마지막으로 검증한 현재 작업을 유지합니다. **상위 작업 1**은 별도의 선택 가능한 액션이므로 사용자 프로필에 두 버튼을 나란히 둘 수 있습니다.
...
1. **대시보드** — 한도, 현재 Codex 작업, Fast mode, 새 작업, 사이드챗, 마이크, 보내기, 뒤로가기. Fast mode는 `1,1`, 현재 작업은 `0,1`에 있습니다.
