...
> [!IMPORTANT]
> 음성 입력은 자동으로 설정되지 않습니다. Codex에서 `Control+Shift+D`를 지정한 뒤 사용하세요. 마이크 버튼을 눌렀을 때 `D`만 입력되거나 아무 반응이 없다면 이 단축키와 Stream Deck의 손쉬운 사용 권한을 먼저 확인하세요.
...
| 액션 | 기본 동작 |
|---|---|
| 작업 열기 | 짧게 누르면 로컬 `codex://` URL로 선택한 작업을 엽니다. 0.55초 이상 누르면 작업을 연 뒤 음성 입력을 시작하고, 버튼을 놓으면 녹음을 끝내 받아쓰기가 안정될 때까지 기다렸다가 일반 후속 요청으로 자동 제출합니다. |
...
| 데이터 소스 | 접근 방식 | 목적 |
|---|---|---|
| `~/.codex` 아래 Codex 파일과 Desktop 로그 | 읽기 전용 | 제목, 고정, 상태, 활동, 시간, 서비스 메타데이터, 임시 사이드챗 수명 주기. |
...
대기열 감지는 네이티브 헬퍼 안에서 현재 Codex 창 제목과 현지화된 대기열 액션 라벨을 해시 처리합니다. 플러그인에는 해시와 개수만 전달되며, 대기 메시지 본문은 반환·기록·저장하지 않습니다. Codex 데이터베이스나 세션 파일에는 절대 쓰지 않습니다. 이슈에 로그나 화면을 첨부하기 전에 [보안과 개인정보 보호 안내](SECURITY.ko.md)를 읽어주세요.
