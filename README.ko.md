...
- 로컬 컴퓨터의 고정·최근 작업 또는 직접 고정한 원격 작업 중 어디로 이동할 것인가?
- 주간 한도가 얼마나 남았는가?
...
직접 고정한 원격 작업은 Codex의 UUIDv7 턴 ID에서 최근 턴 시작 시각을 복원하고, 명시적인 종료 수명 주기 이벤트 또는 실시간으로 확인한 `active` → `notLoaded` 전환에서만 타이머를 멈춥니다. 원격 요약의 업데이트 시각은 정렬에만 사용하며 완료 시각으로 쓰지 않습니다. 플러그인을 새로 시작해 신뢰할 종료 경계를 얻을 수 없으면 짧은 시간을 만들어 내지 않고 소요 시간을 알 수 없음으로 표시합니다. 작업 중에는 현재 턴의 reasoning-summary 선행 동사를 원문 노출 없이 `계획 중`, `분석 중`, `구현 중`, `검증 중`, `실행 중`, `정리 중` 같은 보수적인 단계로 표시합니다. 단계가 오래됐거나 현재 턴과 일치하지 않으면 `원격 작업`으로 안전하게 되돌아갑니다. 추론 강도는 포커스된 창이 정확한 원격 작업과 일치할 때 보이는 작성기의 실제 설정값(또는 명시적인 원격 메타데이터)만 사용합니다. Codex가 값을 노출하지 않으면 중간으로 추정하지 않고 트랙을 비워 둡니다.
...
5. Codex Desktop의 **설정 → 키보드 단축키**에서 **받아쓰기 시작**이 `⌃⇧D`인지 확인합니다. 현재 Codex의 기본값입니다.
...
|---|---:|---|
| 받아쓰기 시작 | `⌃⇧D` | 전용 마이크 버튼, 작업 버튼 길게 누르기 |
| 프로젝트 밖에 새 작업 열기 | `⌥⌘O` | 새 작업 버튼 |
| 사이드챗 열기 | `⌥⌘S` | 사이드챗 버튼 |

> [!IMPORTANT]
> 현재 Codex는 **받아쓰기 시작**에 `Control+Shift+D`를 기본 지정합니다. 직접 바꿨다면 이 값으로 되돌리세요. ThreadDeck이 라틴 `D`를 명시해 보내므로 2벌식 한글을 비롯한 비라틴 입력 소스에서도 작동합니다. 앱 단축키는 키를 뗀 이벤트를 무시하므로, ThreadDeck은 버튼을 놓을 때 화면의 음성 입력 중지 컨트롤을 직접 활성화합니다.
...
| 액션 | 기본 동작 |
|---|---|
| 작업 열기 | 로컬 작업은 짧게 누르면 `codex://`로 엽니다. 원격 작업은 제목이 정확히 일치하는 Codex 사이드바 또는 통합 검색 컨트롤에 포커스를 준 뒤 키보드로 활성화하므로 합성 마우스 좌표나 포인터 이동 없이 해당 작업의 컴퓨터까지 전환합니다. 0.55초 이상 누르면 작업을 연 뒤 음성 입력을 시작하고, 버튼을 놓으면 녹음을 끝내 받아쓰기가 안정될 때까지 기다렸다가 일반 후속 요청으로 자동 제출합니다. |
| 새 작업 | `⌥⌘O`를 보내 현재 프로젝트 밖에 새 작업을 엽니다. |
| 사이드챗 | `⌥⌘S`를 보냅니다. |
| 누르는 동안 음성 입력 | `⌃⇧D`로 음성 입력을 시작하고, Stream Deck 버튼을 놓으면 Codex의 음성 입력 중지 컨트롤을 활성화합니다. 소리를 출력 중인 앱은 누르는 동안 잠시 멈췄다가 다시 시작합니다. |
...
| 데이터 소스 | 접근 방식 | 목적 |
|---|---|---|
| `~/.codex` 아래 Codex 파일과 Codex Desktop 로그 | 읽기 전용 | 로컬 작업과 캐시된 원격 작업의 제목·호스트·고정·상태·활동·시간·서비스 메타데이터, 임시 사이드챗 수명 주기. |
| CodexBar CLI | 선택적 하위 프로세스 | 남은 주간 한도만 확인. |
| Stream Deck 플러그인 WebSocket | 로컬호스트 | 버튼 이벤트 수신과 렌더링 이미지 전송. |
| macOS 손쉬운 사용 / Core Audio | 로컬 시스템 API | 단축키, 미디어 키, 음성 입력 중 오디오 처리, 포커스된 작성기의 고정 추론 단계 값, 화면에 보이는 Codex 대기열 액션 개수 확인. |

대기열 감지, 포커스된 작업 확인, 원격 작업 선택은 네이티브 헬퍼 안에서 제목 지문을 비교합니다. 원격 제목은 명령행 인수가 아니라 표준 입력으로 Codex 검색에 전달합니다. 대기 메시지 본문과 임의의 손쉬운 사용 텍스트는 반환·기록·저장하지 않습니다. Codex 데이터베이스나 세션 파일에는 절대 쓰지 않습니다. 이슈에 로그나 화면을 첨부하기 전에 [보안과 개인정보 보호 안내](SECURITY.ko.md)를 읽어주세요.
