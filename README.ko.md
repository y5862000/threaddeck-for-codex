...
직접 고정한 원격 작업은 Codex의 UUIDv7 턴 ID에서 최근 턴 시작 시각을 복원하고, 원격 요약의 서버 업데이트 시각에서 타이머를 멈춥니다. 추론 강도는 포커스된 창이 정확한 원격 작업과 일치할 때 보이는 작성기의 실제 설정값(또는 명시적인 원격 메타데이터)만 사용합니다. Codex가 값을 노출하지 않으면 중간으로 추정하지 않고 트랙을 비워 둡니다.
