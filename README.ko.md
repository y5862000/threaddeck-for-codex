...
| 추론 강도 + Fast | 0.6초 전에 놓으면 다음 응답의 단계명(`LIGHT`–`ULTRA`)을 즉시 표시하고 적용 | Codex에서 직접 바꾼 값도 이 버튼에 반영. 없는 단계는 건너뛰고 양 끝에서 방향 반전하며 Codex의 작은 슬라이더 UI와 Effort 메뉴를 모두 지원. Full access에서 Codex의 정확한 Ultra 경고가 뜨면 대화상자를 검증하고 `Use Full access`를 선택하며, 권한을 바꾸는 일반 `Continue` 경로는 누르지 않음. **0.6초** 이상 누르는 순간 버튼을 놓기 전에 다음 응답의 Fast mode 전환을 시작. 작은 번개는 확인된 속도 표시 |
